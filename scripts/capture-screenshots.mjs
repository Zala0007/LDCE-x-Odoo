...
const trip = await db.trip.findFirstOrThrow({
  where: { owner: { email: "he.demo@globetrotter.local" }, name: "Japan Spring Stories" },
...
try {
  ...
  await publicPage.goto(`${origin}/share/${trip.publicLink.slug}`, { waitUntil: "networkidle" });
  await publicPage.evaluate(() => window.scrollTo(0, 0));
  await publicPage.screenshot({ path: path.join(output, "public-itinerary.png"), fullPage: false });
  await publicContext.close();

  const mobile = await browser.newContext({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 1 });
  const mobilePage = await mobile.newPage();
  await mobilePage.goto(`${origin}/login`);
  await mobilePage.getByLabel("Email address").fill("he.demo@globetrotter.local");
  ...
  await mobilePage.screenshot({ path: path.join(output, "dashboard-mobile.png"), fullPage: false });
  await mobile.close();

  console.log(`Captured ${pages.length + 2} screenshots in ${output}.`);
} finally {
  await browser.close();
  server.kill("SIGTERM");
}
