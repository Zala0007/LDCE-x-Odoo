...
type CalendarStop = {
  ...
  endDate: Date;
  city: { name: string };
  itineraryActivities: CalendarActivity[];
};
  ...
  return (
    <div>
      ...
      {mode === "calendar" ? (
        ...
        <div className="calendar-timeline-list">
          {initialStops.map((stop) => (
            <StopTimelineOrder key={stop.id} tripId={tripId} stop={stop} canEdit={canEdit} />
          ))}
          {!initialStops.length ? (
            <p className="activity-empty">
              Add a stop in the builder to begin the timeline.
            </p>
          ) : null}
        </div>
      )}
      <div className="calendar-edit">
        ...
        </Link> : <span>Read-only shared itinerary</span>}
        <span>{days.length} travel days</span>
      </div>
    </div>
  );
}
      ...
      <div className="calendar-grid">
        ...
        {monthDays.map((day) => {
          ...
          const dayStops = stops.filter(
            (stop) =>
              day >= new Date(stop.startDate) && day <= new Date(stop.endDate),
          );
          const activities = dayStops.flatMap((stop) =>
            stop.itineraryActivities.filter((item) =>
          ...
          return (
            <article
              key={day.toISOString()}
              className={inTrip ? "calendar-day in-trip" : "calendar-day"}
            >
              <span className="calendar-number">{format(day, "d")}</span>
              {dayStops.slice(0, 2).map((stop) => (
                <small key={stop.id}>
                  <MapPin size={10} />
                  {stop.city.name}
                </small>
              ...
              {activities.length ? (
                <strong>
                  {activities.length}{" "}
                  {activities.length === 1 ? "plan" : "plans"}
                </strong>
              ) : null}
            </article>
          );
...
function StopTimelineOrder({
  ...
  useEffect(
    () => setItems(stop.itineraryActivities),
    [stop.itineraryActivities],
  );
  ...
  return (
    <details className="timeline-stop" open>
      <summary>
        <span>
          <MapPin size={17} />
          {stop.city.name}
        ...
        >
          ...
          >
            <div
              className={
                pending
                  ? "timeline-sort-list builder-saving"
                  : "timeline-sort-list"
              }
            >
              {items.map((item) => (
                <SortableCalendarActivity key={item.id} item={item} canEdit={canEdit} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
      ) : (
        <p className="activity-empty">No activities planned for this stop.</p>
      )}
    </details>
  );
}

function SortableCalendarActivity({ item, canEdit }: { item: CalendarActivity; canEdit: boolean }) {
  ...
  return (
    ...
    >
      <button
        type="button"
        disabled={!canEdit}
        aria-label={canEdit ? `Reorder ${item.activity.name}` : `${item.activity.name} order`}
        {...attributes}
        {...listeners}
      >
      ...
      <time>
        <b>{format(new Date(item.date), "MMM d")}</b>
        <span>
          <Clock3 size={12} />
      ...
      <div>
        ...
        <p>
          {item.activity.category} · {item.activity.durationMinutes} minutes
        </p>
      </div>
      <em>{formatCurrency(item.cost ?? item.activity.estimatedCost)}</em>
    </article>
  );
}
