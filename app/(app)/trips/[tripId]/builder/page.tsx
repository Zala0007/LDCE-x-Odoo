...
import { getOwnedItinerary, listCityChoices } from "@/lib/repositories/itinerary-repository";
...
export default async function BuilderPage({ params, searchParams }: { params: Promise<{ tripId: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const [trip, cities] = await Promise.all([getOwnedItinerary(tripId, session.user.id), listCityChoices()]);
  if (!trip) notFound();
