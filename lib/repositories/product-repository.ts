...
export function listCommunityPosts(
  filters: {
    q?: string;
    sort?: "active" | "recent" | "oldest";
    country?: string;
    scope?: "trip" | "standalone";
  } = {},
) {
  ...
  if (filters.country) where.author = { country: filters.country };
  if (filters.scope === "trip") where.tripId = { not: null };
  if (filters.scope === "standalone") where.tripId = null;
  const orderBy:
    ...
    include: {
      ...
      trip: {
        select: {
          id: true,
          name: true,
          publicLink: { select: { slug: true, isActive: true } },
        },
      },
    },
    orderBy,
...
export async function adminAnalytics(q = "") {
  ...
  const [
    ...
    db.activity.findMany({
      take: 6,
      orderBy: { itineraryUses: { _count: "desc" } },
      select: {
        name: true,
        category: true,
        _count: { select: { itineraryUses: true } },
      },
    }),
    db.user.findMany({
      where: q
  ...
  for (const trip of tripsByMonth) {
    const key = trip.createdAt.toLocaleDateString("en-US", {
      month: "short",
      year: "2-digit",
    });
  ...
  return {
    ...
    popularActivities,
    users,
    tripsOverTime: [...monthMap].map(([month, trips]) => ({ month, trips })),
  };
}
